...
//start at index 0 just like a string. "bulls" is at index 0
var basketballTeams = ["bulls", "hawks", "rockets", "knicks", "celtics", "warriors", "hornets", "cavs", "spurs", "heat"];
basketballTeams.push("nets");
basketballTeams.push("raptors");

var basketballTeamsWithRs = [];
var basketballTeamsWithoutRs = [];
...
//start with for
//define var i, which starts at index 0, meaning it is starting at the beginning of the array
...
for(var i = 0; i < basketballTeams.length; i++){
	if(basketballTeams[i].indexOf("rs") > -1){
		...
		basketballTeamsWithoutRs.push(basketballTeams[i])
	}
}
...
console.log(basketballTeamsWithoutRs);

console.log(basketballTeams[1].charAt(0));
//Here i am looping through the array and logging each string with the first character as toUpperCase
for(var i = 0; i < basketballTeams.length; i++){
	console.log(basketballTeams[i].charAt(0).toUpperCase() + basketballTeams[i].substring(1, basketballTeams[i].length))
}
